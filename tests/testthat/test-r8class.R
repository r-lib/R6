...
test_that("Private members are private, and self/private environments", {
  ...
  expect_identical(ls(private_bind_env), "y")
  expect_identical(parent.env(private_bind_env), emptyenv())

  # Eval environment
  eval_env <- attr(A, "eval_env")
  ...
  # The public methods environment
  public_methods_env <- attr(A, "methods")
  expect_identical(
    sort(ls(public_methods_env)),
    c("getprivate", "getself", "getx", "getx2", "gety")
  )
  expect_identical(parent.env(public_methods_env), emptyenv())

  # Environment of public methods should be same as eval_env - but only when
  # fetched with A$getx.
  expect_identical(environment(A$getx), eval_env)
  # When fetched directly from the public_methods_env, it should have this
  # environment.
  expect_identical(environment(public_methods_env$getx), environment())

  # The private methods environment
  private_methods_env <- attr(A$getprivate(), "methods")
  expect_identical(ls(private_methods_env), "getx_priv")
  expect_identical(parent.env(private_methods_env), emptyenv())

  # Environment of private methods should be same as eval_env - but only when
  # fetched with private$getx_priv.
  expect_identical(environment(A$getprivate()$getx_priv), eval_env)
  # When fetched directly from the public_methods_env, it should have this
  # environment.
  expect_identical(environment(private_methods_env$getx), environment())
...
test_that("Private methods exist even when no private fields", {
  ...
  # The private binding environment contains private fields
  private_bind_env <- A$getprivate()
  expect_identical(length(private_bind_env), 0L)
  expect_identical(parent.env(private_bind_env), emptyenv())
  ...
  # Environment of private methods should be same as eval_env - but only when
  # fetched with private$getx_priv.
  expect_identical(environment(A$getprivate()$getx_priv), attr(A, "eval_env"))
  # When fetched directly from the public_methods_env, it should have this
  # environment.
  expect_identical(environment(private_methods_env$getx), environment())
})
...
test_that("Validity checks on creation", {
  ...
  expect_error(R8Class("AC", public = list(1)))
  expect_error(R8Class("AC", private = list(1)))
  expect_error(R8Class("AC", active = list(fun)))

  # Names can't be duplicated
  expect_error(R8Class("AC", public = list(a=1, a=2)))
  expect_error(R8Class("AC", public = list(a=1), private = list(a=1)))
  expect_error(R8Class("AC", private = list(a=1), active = list(a=fun)))

  # Reserved names
  expect_error(R8Class("AC", public = list(self = 1)))
  expect_error(R8Class("AC", private = list(private = 1)))
  expect_error(R8Class("AC", active = list(super = 1)))

  # `initialize` only allowed in public
  expect_error(R8Class("AC", private = list(initialize = fun)))
  expect_error(R8Class("AC", active = list(initialize = fun)))
})
...
test_that("Inheritance", {
  ...
  # Environment checks
  eval_env <- attr(B, "eval_env")
  super_bind_env <- eval_env$super
  super_eval_env <- attr(super_bind_env, "eval_env")
  ...
  # Classes
  expect_identical(class(B), c("BC", "AC", "R8_shared", "R8"))
})
...
test_that("Inheritance: superclass methods", {
  ...
  # Classes
  expect_identical(class(C), c("CC", "BC", "AC", "R8_shared", "R8"))
})
