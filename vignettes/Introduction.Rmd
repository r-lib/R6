...
If your R6 class contains any fields that also have reference semantics (e.g., other R6 objects, environments), those fields should be populated in the `initialize` method. If the field is directly set to the reference object in the class definition, that object will be shared across all instances of the R6 objects. Here's an example:
