...
**NOTE:** In R6 version 2.3.0 (released 2018-10-04) and below, the `finalize` method could only be public. As of version 2.4.0, it could be public or private. (Private is preferable because there's no reason a finalizer needs to be publicly accessible.) In future versions of R6, it will emit a message if a finalizer is public; later, it will emit warnings, and finally it will no longer support public finalizers.
