...
To instantiate an object of this class, use `$new()` method:
...
If your R6 class contains any fields that also have reference semantics (e.g., other R6 objects, environments, etc.), those fields should be populated in the `initialize` method. If the field is directly set to the reference object in the class definition, that object will be shared across all instances of the R6 objects. Here's an example:
...
As of R6 2.5.0, if the object is cloned, member (non-method) functions will not have their enclosing environment changed, which is what one would normally expect. It will behave this way:
