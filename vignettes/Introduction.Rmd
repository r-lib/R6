...
Here's how to create a simple R6 class:

```{r}
library(R6)

Person <- createR6Class("Person",
  public = list(
    name = NA,
    hair = NA,
...
Unless there's a compelling reason otherwise, methods should return `self` because it makes them chainable. For example, `add()` is returns `self` so you can chain them together:

```{r}
q$add(10)$add(11)$add(12)
```

On the other hand, the `remove()` returns the value removed, so it's not chainable:
...
HistoryQueue <- createR6Class("HistoryQueue",
  inherit = Queue,
  public = list(
    show = function() {
...
```{r}
CountingQueue <- createR6Class("CountingQueue",
  inherit = Queue,
  public = list(
    add = function(x) {
      total <<- total + 1
...
R6 classes provide many capabilities common to object-oriented programming languages. They're similar to R's built-in reference classes, but they are simpler, smaller, faster, and easier to understand.
