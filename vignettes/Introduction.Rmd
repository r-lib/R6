...
1. If the superclass has turned off cloning (`cloneable=FALSE`), the subclass will also not be cloneable. That is, subclass will not have a cloning method even if it has set `cloneable=TRUE`.

For example, let's create R6 class called `Creature` with `cloneable` set to `FALSE`.
...
If we now create R6 subclass `Human` that inherits from `Creature` class, even if we set `cloneable=TRUE`, cloning instances of this subclass will still not be possible because the superclass disallows cloning:
...
2. If the superclass has `cloneable=TRUE`, then cloning method will be decided by the subclass.

Let's use the same class `Creature` defined above, but now with `cloneable` set to `TRUE`.
...
Now whether subclass `Human` that inherits from `Creature` class will have a cloning method is decided by `cloneable` argument.

If it is set to `TRUE`, the subclass will have a cloning method.
