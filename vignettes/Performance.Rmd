...
R reference classes are much slower to instantiate than the other types of classes, with a median of `r speed$median[speed$name == "A_rc"] / 1000` milliseconds per instantiation.
...
For reference and R6classes, assignment using `.self$x <-` is somewhat slower than using `x <<-`.
...
# Define an S3 method for class bar
`$.e3` <- function(x, name) {
  NULL
}
```

Now we can run timing tests for calling `$` on each type of object. Note that for the `e3` object of class `"bar"`, the `$` function does nothing --- it simply returns `NULL`.
...
The `[[` operator is slightly faster than `$`, probably because it doesn't need to convert the unevaluated symbol to a string; it doesn't need to convert the unevaluated symbol to a string.
