...
p <- ggplot(speed, aes(y = name, x = median)) +
  ...
  geom_point(size = 2) +
  my_theme +
  ggtitle("Median time to call a method that accesses a field (\u0b5s)")
...
R6 objects take less memory and are significantly faster than R's reference class objects, and they also have some options that provide for even more speed. Reference classes do provide additional features, such as type checking of fields, but they also have other drawbacks --- notably, they are built using S4 classes and so they bring with them issues with using S4.

Another major advantage of R6 objects is that they are much simpler than R's reference class objects: they are essentially just environments.
