...
```{r}
C_wrc_priv <- createRefClass("C_wrc_priv",
  private = list(x = NULL),
  public = list(
...
microbenchmark(
  A_rc = A$x,
  B_wrc = B$x,
  C_wrc_priv = C$private$x,
  D_closure_class = D$x,
  E_closure_noclass = E$x,
  unit = "us"
)
```
...
microbenchmark(
  A_rc = A$x <- 4,
  B_wrc = B$x <- 4,
  C_wrc_priv = C$private$x <- 4,
  D_closure_class = D$x <- 4,
  E_closure_noclass = E$x <- 4,
  unit = "us"
)
```

The difference between `D` and `E` is due to overhead from the `class` attribute. Because `D` has a class, R will check whether there is a `$` method for its class. All of the objects `A`--`D` have a class, while `E` does not.
