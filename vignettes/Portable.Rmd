...
ClassA <- R6Class("ClassA",
  portable = FALSE,  # This is the default
  public = list(
    foo = function() fun()
  ),
  parent_env = pkgA
...
R6 does support inheritance across different packages, with the `portable=TRUE` option. In this example, we'll again simulate different packages by creating separate parent environments for the classes.
...
One important difference between non-portable and portable classes is that with non-portable classes, it's possible to access members with just the name of the member, and with portable classes, member access always requires using `self$` or `private$`. 

Here's an example of a non-portable class with two methods: `sety`, which sets the private field `y`, and `getxy`, which returns a vector with teh values of fields `x` and `y`:
...
* Portable classes always require the use of `self` or `private` to access members.
