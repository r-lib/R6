...
A new environment uses `r as.numeric(object_size(new.env()))` bytes, regardless of what the parent environment is.
...
```{r}
microbenchmark(new.env(), unit = "us")
```
...
parse_string_short <- function() parse(text = short_string)
...
parse_string_long <- function() parse(text = long_string)
...
We can also compare the length of a 
